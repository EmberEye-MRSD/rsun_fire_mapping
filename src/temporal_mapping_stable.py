...
class Filter:
    def __init__(self):
        ...
        # hotspot_inflation_radius_param = rospy.get_param('/hotspot_inflation_radius', default=1.0)
    ...
    def update_global_poses(self, poses_reading):
        ...
        #     print("Number of global hotspots : ", len(self.global_poses), np.array(self.global_poses[0]).shape)
        #     plt.scatter(np.array(self.global_poses)[:, 0], np.array(self.global_poses)[:, 2], c='g')
        #     for i in range(len(self.global_poses)):
        #         print(f"Measurements for hotspot {i} : ", len(self.measurements[i]))
        #         plt.scatter(np.array(self.measurements[i])[:, 0], np.array(self.measurements[i])[:, 2], c='r')
