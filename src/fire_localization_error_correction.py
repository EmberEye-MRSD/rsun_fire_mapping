...
# magic_func = np.load('magic_func_gangu.npy')
# magic_func = np.load('magic_func_ishir.npy')
fixmy_errors = np.load('fixmy_errors.npy')
...
publish_feature_left = True
publish_centre_left = True
...
class FireLocalization:
   ...
   def do_magic(self, depth, magic_func):

      print("Depth is: ", depth)
      print("Magic Func is: ", magic_func)
      print (np.polyval(magic_func, depth))
      return np.polyval(magic_func, depth)
   
   def do_more_magic(self, pose_imu_3d, magic_func):
      # pass
      # print("Depth is: ", depth)
      # print("Magic Func is: ", magic_func)
      # print (np.polyval(magic_func, depth))
      # return np.polyval(magic_func, depth
      fixmy_x = fixmy_errors[0]
      fixmy_y = fixmy_errors[1]
      fixmy_z = fixmy_errors[2]

      dist = np.linalg.norm(pose_imu_3d)
      x = pose_imu_3d[0]
      y = pose_imu_3d[1]
      z = pose_imu_3d[2]

      x -= np.polyval(fixmy_x, x)
      y -= np.polyval(fixmy_y, y)
      z -= np.polyval(fixmy_z, z)
   ...
   def getHotspotsfrommatches(self, left_coords, right_coords):
      ...
      # depth += 0.34
      # depth = depth - self.do_magic(depth, magic_func) - 0.34
      ...
      pos_imu_3d = self.do_more_magic(pos_imu_3d, fixmy_errors)

      pos_3d = (self.H_imu_world@pos_imu_3d)[:3]

      return np.array(pos_3d) , np.array(pos_imu_3d[:3])
   ...
   def main_loop(self, event):
      ...
      for i in range(mismatched_length):
         ...
         if np.linalg.norm(pose_imu_3d) < 8:
            hotspots_global_frame.append(pose_3d)
            total_poses.append(pose_3d)
            total_drone_poses.append(pose_imu_3d)
...
if __name__ == "__main__":
   ...
   while rospy.is_shutdown():
      ...
      # np.save("total_poses_nardoconfig2bag.npy", total_poses)
      # np.save("disp_config2", all_disp)
      # np.save("raw_data_config2.npy", [total_poses,total_drone_poses])
      # np.save("config2.npy", [total_poses,total_drone_poses])
      # np.save("nardoconfig2.npy", [total_poses,total_drone_poses])
      np.save("fitted_plot.npy", [total_poses, total_drone_poses])
      exit()